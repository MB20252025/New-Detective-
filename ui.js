...
// ============================================================
//  OPTION SANITISING — used by the Main Puzzles tab
// ============================================================
// Aggressively normalises a string: lowercase, only a-z0-9.
...
// Strips a leading "A) ", "A. ", "A - ", "A: " prefix from option text
// when the letter matches.
function stripLetterPrefix(letter, text) {
  var t = (text || '').toString().replace(/\s+/g, ' ').trim();
  if (!letter) return t;
  var re = new RegExp('^' + letter + '\\s*[\\)\\.\\-:\\s]\\s*', 'i');
  return t.replace(re, '').trim();
}

// Takes raw options from the game data, returns a clean array of {letter, text}
// that is deduped by normalised text and re-lettered A, B, C in order.
function sanitiseOptions(raw) {
  if (!Array.isArray(raw)) return [];

  // 1. Parse each option into {letter, text}
  var parsed = [];
  for (var i = 0; i < raw.length; i++) {
    ...
    if (typeof opt === 'string') {
      ...
      letter = (opt.letter || opt.label || opt.key || '').toString().trim().toUpperCase();
      text = (opt.text || opt.value || opt.answer || '').toString();
      if (!text && opt.label) text = opt.label.toString();
    }
    ...
    if (letter) {
      ...
      // Try to sniff the letter out of the text
    ...
    parsed.push({ letter: letter, text: text });
  }

  // 2. Dedupe by normalised text
  var seen = {};
  var unique = [];
  for (var j = 0; j < parsed.length; j++) {
    var key = normaliseKey(parsed[j].text);
    if (!key || seen[key]) continue;
    seen[key] = true;
    unique.push(parsed[j]);
  }

  // 3. Cap at 3 (every puzzle in this game has exactly 3 options)
  unique = unique.slice(0, 3);

  // 4. Re-letter A, B, C
  for (var k = 0; k < unique.length; k++) {
    unique[k].letter = String.fromCharCode(65 + k);
  }

  return unique;
}

// Returns the sanitised text of the correct answer for a puzzle.
function getCorrectTextForPuzzle(puzzle) {
  if (!puzzle) return '';
  var sol = (puzzle.solution || '').toString().toUpperCase();
  var opts = Array.isArray(puzzle.options) ? puzzle.options : [];
  for (var i = 0; i < opts.length; i++) {
    var o = opts[i];
    if (!o) continue;
    var letter = (o.letter || '').toString().toUpperCase();
    if (letter === sol) {
      return stripLetterPrefix(letter, (o.text || '').toString());
    }
  }
  return '';
}
...
function buildMainPuzzlesState() {
  ...
  // ---- Sanitise options ONCE, here ----
  var cleanOptions = sanitiseOptions(displayPuzzle.options);
  var digitText = getCorrectTextForPuzzle(displayPuzzle);
...
// Listen for messages from the main-puzzles iframe
window.addEventListener('message', function (event) {
  var d = event.data;
  if (!d || typeof d !== 'object') return;
  ...
  if (d.type === 'ANSWER') {
    ...
    var correctText = getCorrectTextForPuzzle(currentP);
    var clickedText = (d.text || '').toString();

    var isCorrect = (normaliseKey(clickedText) === normaliseKey(correctText));

    console.log('[mainPuzzles] clicked:', clickedText, '| correct:', correctText, '| match:', isCorrect);
