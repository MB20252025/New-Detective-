...
// ============================================================
//  RENDER: INTERVIEWS TAB (new Interrogation Dashboard)
// ============================================================
...
function renderInterviewsContent() {
  ...
  // Build snapshot of state the dashboard needs
  var snapshot = {
  ...
  (game.suspects || []).forEach(function (s) {
    questionsPayload[s.id] = { questions: s.questions, responses: s.responses };
  });

  content.innerHTML =
    '<div style="width:100%; height:100%; min-height:560px; background:#060a10; border-radius:16px; overflow:hidden;">' +
  ...
  interviewsIframe.onload = function () {
    sendInterviewsState(snapshot, questionsPayload);
  };
}

function sendInterviewsState(snapshot, questionsPayload) {
...
window.addEventListener('message', function (event) {
  ...
  // User asked a question in the iframe
  if (d.type === 'ASK_QUESTION') {
    var sid = d.suspectId;
    ...
    // Phase 1 completion: 3 questions to Neil Down → unlock Main Puzzles & Bonus Puzzles
    ...
    // Push updated state back into the iframe so it re-renders
    if (interviewsIframe && interviewsIframe.contentWindow) {
      var updated = {
        initialInterviewDone: !!game.initialInterviewDone,
        mainSolved: game.mainSolved || [],
        bonusSolved: game.bonusSolved || [],
        askedQuestions: JSON.parse(JSON.stringify(game.askedQuestions)),
        suspectStress: JSON.parse(JSON.stringify(game.suspectStress))
      };
      interviewsIframe.contentWindow.postMessage({ type: 'INIT_STATE', data: updated }, '*');
    }
  }
});
