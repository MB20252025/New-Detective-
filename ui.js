...
function buildMainPuzzlesState() {
  ...
  if (current) {
    // There is an unsolved puzzle currently available
    displayIdx = game.mainPuzzles.findIndex(function(p){ return p.id === current.id; });
    isInteractive = true;
  } else {
    // No available puzzle — show the most recently solved one
    var lastSolved = -1;
    for (var i = 0; i < game.mainSolved.length; i++) {
      if (game.mainSolved[i]) lastSolved = i;
    }
    if (lastSolved >= 0) {
      displayIdx = lastSolved;
      isInteractive = false;
    } else {
      // Fresh game, nothing solved yet — show puzzle 1 as locked
      displayIdx = 0;
      isInteractive = false;
    }
  }
  ...
  // Correct option letter (from solution)
  var digitLetter = displayPuzzle.solution;
...
function renderMainPuzzles() {
  ...
  // Taller wrapper — no header inside means more vertical room for the card
  panel.innerHTML =
