...
function closeAllModals() {
  ...
  iframeModals.forEach(function(el) {
    if (el.id !== 'startMenu' && el.id !== 'gameWrapper') el.remove();
  });
...
function parseOption(raw, index) {
  ...
  if (letter) { letter = letter.toUpperCase(); text = stripLetterPrefix(letter, text); }
  else {
    var m = text.match(/^([A-Z])\s*[\)\.\-:]\s*/i);
    if (m) { letter = m[1].toUpperCase(); text = text.substring(m[0].length).trim(); }
  }
...
function sanitiseOptions(raw) {
  ...
  for (var x = 0; x < unique.length; x++) unique[x].letter = String.fromCharCode(65 + x);
  return unique;
}
...
// ============================================================
//  CASE FILE TAB — the folder grid
// ============================================================
function generateFolderContents() {
  // Detective notes preview (used only as a fallback if the casebook isn't available)
  ...
  var interviewResults = '📋 Interview transcripts — click a suspect to review what they said.';
  ...
  return { notes: notesHtml, interviewResults: interviewResults, puzzleLog: puzzleLog, suspectsFolder: suspectsFolder };
}
...
function renderCaseFile() {
  ...
  panel.innerHTML =
    '<div class="folder-grid">' +
      ...
      '<div class="folder-card" data-folder="interviewResults"><span class="folder-icon">🗣️</span><div class="folder-title">Interview Results</div></div>' +
      ...
      '<div class="folder-card" data-folder="newspaper"><span class="folder-icon">📰</span><div class="folder-title">Daily Newspaper</div></div>' +
      ...
      '<div class="folder-card" data-folder="mainmenu"><span class="folder-icon">🏠</span><div class="folder-title">Main Menu</div></div>' +
    '</div>';
  ...
  panel.querySelectorAll('.folder-card').forEach(function(card) {
    card.addEventListener('click', function() {
      ...
      if (folder === 'detectiveNotes') {
        // Open the casebook modal if available
      ...
      else if (folder === 'interviewResults') {
      ...
      else if (folder === 'newspaper') {
        // Switch to the Newspaper tab
        if (game) {
          game.currentTab = 'newspaper';
          var newspaperTab = document.querySelector('.tab[data-tab="newspaper"]');
          if (newspaperTab) {
            document.querySelectorAll('.tab').forEach(function(t){ t.classList.remove('active'); });
            newspaperTab.classList.add('active');
          }
          renderNewspaperTab();
        }
      }
      ...
      else if (folder === 'mainmenu') {
        if (game) game.saveToLocalStorage();
        document.getElementById('gameWrapper').style.display = 'none';
        document.getElementById('startMenu').style.display = 'flex';
        document.querySelectorAll('.tab').forEach(function(t){ t.classList.remove('active'); });
        var mp = document.querySelector('.tab[data-tab="casefile"]');
...
function showInterviewResults() {
  var available = ['s1'];
  if (game.initialInterviewDone) available.push('s2', 's3');

  var html = '<div><h3>Select a suspect to view their interview transcript:</h3>';
  available.forEach(function(sid) {
    var suspect = game.suspects.find(function(s){ return s.id === sid; });
    if (!suspect) return;
    var asked = game.askedQuestions[sid] || [];
    var stress = game.suspectStress[sid] || 0;
    var stressColor = stress > 80 ? '#d9534f' : (stress > 50 ? '#f0ad4e' : '#5cb85c');
    html += '<div class="suspect-folder-item" data-suspect="' + sid + '" style="margin-bottom:16px; cursor:pointer;">' +
      '<div style="display:flex; justify-content:space-between; align-items:center;">' +
        '<strong>' + suspect.emoji + ' ' + suspect.name + '</strong>' +
        '<span style="background:#2a2a2a; padding:2px 8px; border-radius:20px; font-size:0.7rem; color:#fff;">Stress: ' + stress + '%</span>' +
      '</div>' +
      '<div style="background:#444; border-radius:10px; height:8px; width:100%; margin:8px 0 4px;">' +
        '<div style="background:' + stressColor + '; width:' + stress + '%; height:8px; border-radius:10px;"></div>' +
      '</div>' +
      '<div style="font-size:0.8rem; color:#000;">' + asked.length + ' questions answered</div>' +
    '</div>';
  });
  html += '</div>';
  showModal("📋 Interview Results", html);

  setTimeout(function() {
    document.querySelectorAll('.suspect-folder-item').forEach(function(item) {
      item.addEventListener('click', function() {
        var sid = item.getAttribute('data-suspect');
        var suspect = game.suspects.find(function(s){ return s.id === sid; });
        if (!suspect) return;
        var asked = game.askedQuestions[sid] || [];
        var transcript = '<div><h3>' + suspect.emoji + ' ' + suspect.name + ' — Full Transcript</h3>';
        if (asked.length === 0) {
          transcript += '<p>No questions asked yet.</p>';
        } else {
          asked.forEach(function(idx) {
            transcript += '<div style="margin:12px 0; padding:10px; background:#fff7e8; border-radius:12px; color:#2c1c0c;">' +
              '<div style="font-weight:bold;">❓ ' + suspect.questions[idx] + '</div>' +
              '<div style="margin-top:6px; color:#2a5f42;">💬 ' + suspect.responses[idx] + '</div>' +
            '</div>';
          });
        }
        transcript += '</div>';
        showModal(suspect.emoji + ' ' + suspect.name, transcript);
      });
    });
  }, 50);
}
...
function buildInterviewQuestions() {
  var q = {};
  (game.suspects || []).forEach(function (s) { q[s.id] = { questions: s.questions, responses: s.responses }; });
  return q;
}
...
window.addEventListener('message', function (event) {
  ...
  if (d.type === 'READY' && d.source === 'interviews') { pushInterviewState(); return; }
...
function buildMainPuzzlesState() {
  ...
  if (current) {
    ...
    var lastSolved = -1;
    for (var i = 0; i < game.mainSolved.length; i++) if (game.mainSolved[i]) lastSolved = i;
...
window.addEventListener('message', function (event) {
  var d = event.data;
  if (!d || typeof d !== 'object') return;
  if (d.type === 'READY' && d.source === 'mainpuzzles') { pushMainPuzzlesState(); return; }
  if (d.type === 'HINT') {
    var current = game.getCurrentMainPuzzle();
    if (!current) return;
...
function renderBonusPuzzles() {
  ...
  for (var i = 0; i < game.bonusPuzzles.length; i++) {
    ...
    if (!unlocked) {
      ...
      if (p.render) html += p.render(solved);
      else if (solved) html += '<div class="puzzle-title">🎁 ' + p.title + '</div><div class="puzzle-solved-badge">✅ SOLVED! Clue added to Detective Notes.</div>';
      else html += '<div class="puzzle-title">🎁 ' + p.title + '</div><div class="puzzle-desc">' + p.desc + '</div>' +
    ...
    newLetterFolder.onclick = function() {
      ...
      createFullScreenConfirm("📜 BONUS PUZZLE", "The Braggers Note – A letter was found at the crime scene. It looks ordinary, but forensic analysis suggests something is written in invisible ink. Are you ready to examine it under UV light?", function() {
        showLetterPuzzleModal(function() {
          if (letterPuzzle && !game.bonusSolved[0]) { letterPuzzle.onSolve(); game.bonusSolved[0] = true; game.solvedBonusPuzzles++; game.updateUI(); renderBonusPuzzles(); }
        });
      });
    };
  }
...
function isVaultUnlocked() {
  ...
  for (var i = 0; i < game.bonusSolved.length; i++) if (!game.bonusSolved[i]) return false;
  ...
  if (tabBar) {
    tabBar.addEventListener('click', function(e) {
      ...
      if (tab.classList.contains('locked')) {
        if (tab.dataset.tab === 'mainpuzzles') alert('🔒 Complete the initial interview to unlock Main Puzzles.');
        else if (tab.dataset.tab === 'bonuspuzzles') alert('🔒 Complete the initial interview to unlock Bonus Puzzles.');
        else alert('🔒 This tab is locked.');
        return;
      }
      closeAllModals();
      document.querySelectorAll('.tab').forEach(function(t){ t.classList.remove('active'); });
      ...
      if (game) { game.currentTab = tabId; game.refreshCurrentTab(); }
    });
  }
});
...
//  GLOBAL STUBS
// ============================================================
function openMainPuzzle(index) { if (!game) return; var p = game.mainPuzzles[index]; if (!p) return; alert('Opening Main Puzzle: ' + p.title); }
function openBonusPuzzle(index) { if (!game) return; var p = game.bonusPuzzles[index]; if (!p) return; alert('Opening Bonus Puzzle: ' + p.title); }
function askQuestion(suspectId) { if (!game) return; alert('Asking questions to ' + suspectId); }
