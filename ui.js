...
// ============================================================
//  RENDER: INTERVIEWS TAB (Interrogation Dashboard)
// ============================================================
...
window.addEventListener('message', function (event) {
  ...
  // Iframe asked for state again (on first load)
  if (d.type === 'READY') {
    pushInterviewState();
    return;
  }
  ...
  // ---------- PHASE 1: Initial round — only Neil Down, max 3 total ----------
  ...
  // ---------- PHASE 2: Active round — any suspect, 3 total ----------
  ...
  // ---------- PHASE 2: No active round — reject the question ----------
  else {
    return;
  }
...
// ============================================================
//  RENDER: MAIN PUZZLES TAB (styled, iframe-based)
// ============================================================
...
function buildMainPuzzlesState() {
  ...
  if (game.solvedMainPuzzles === game.mainPuzzles.length) {
    return {
      state: 'all_solved',
      currentIndex: game.mainPuzzles.length - 1,
      totalPuzzles: game.mainPuzzles.length
    };
  }

  var current = game.getCurrentMainPuzzle();

  // Case 2: no current puzzle → locked
  if (!current) {
    var lockTitle = 'Next main puzzle locked';
    var lockMessage = 'Complete all required steps to unlock the next puzzle.';

    if (game.waitingForQuestions) {
      lockTitle = 'Next main puzzle locked';
      lockMessage = 'You have ' + game.roundQuestionsRemaining +
        ' question(s) remaining in the current round. Ask them in the Interviews tab.';
    } else {
      var neededBonus = game.getRequiredBonusNameForNextQuestions();
      if (neededBonus) {
        lockTitle = 'Next main puzzle locked';
        lockMessage = 'You need to solve the bonus puzzle "' + neededBonus +
          '" before the next main puzzle will appear. Go to the Bonus Puzzles tab.';
      } else if (!game.bonusSolved[5] && game.solvedMainPuzzles === 6) {
        lockTitle = 'Next main puzzle locked';
        lockMessage = 'You need to solve the Scale & Weight Puzzle in the Bonus Puzzles tab before the final main puzzle appears.';
      }
    }
    return {
      state: 'locked',
      currentIndex: game.solvedMainPuzzles,
      totalPuzzles: game.mainPuzzles.length,
      lockTitle: lockTitle,
  ...
  // Case 3: active puzzle
  var idx = game.mainPuzzles.findIndex(function(p){ return p.id === current.id; });
  var solved = !!game.mainSolved[idx];
  var usedHints = game.hintsUsed[idx] || 0;
...
function renderMainPuzzles() {
  ...
  // Reuse existing iframe if still in DOM (avoids reload flicker)
  ...
  panel.innerHTML =
    '<div style="width:100%; height:calc(100vh - 280px); min-height:520px; max-height:700px; background:#0b1a2b; border-radius:16px; overflow:hidden;">' +
...
window.addEventListener('message', function (event) {
  ...
  if (d.type === 'ANSWER') {
    ...
    if (game.mainSolved[idxP]) return; // already solved

    if (d.letter === currentP.solution) {
      // Correct: show green feedback, then apply solve
